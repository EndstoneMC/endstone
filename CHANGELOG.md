...
- `PlayerOpenSignEvent` for a player beginning to edit a sign's text, reporting the `sign` involved, the `side` being edited and the `cause` (`PLACE`, `INTERACT`, `PLUGIN` or `UNKNOWN`). Cancelling it stops the sign editor from opening. Like Paper's event, it is called before the editor opens, so the sign is already placed when it fires for `PLACE`.
