...
- Added `PlayerSetSpawnEvent`, called when a player's respawn point is set, reporting the `cause` (`BED`, `RESPAWN_ANCHOR`, `COMMAND`, `PLUGIN` or `UNKNOWN`) and the `location` it is being set to. Assigning `location` redirects the respawn point, and assigning `None` removes it. Unlike Bukkit's event this one is not cancellable: Bedrock's `/spawnpoint` and respawn-anchor paths report success without consulting the setter, so a cancel could not be honoured on either. It also does not fire when Bedrock *clears* a respawn point, so `/clearspawnpoint` and breaking the bed a player is bound to are both silent.
