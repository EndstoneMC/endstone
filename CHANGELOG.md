...
- Added a `CreatureSpawner` block state for reading and modifying monster spawners: `spawned_type`, `delay`, `min_spawn_delay`, `max_spawn_delay`, `spawn_count`, `max_nearby_entities`, `required_player_range` and `spawn_range`, all matching their Bukkit counterparts.
- Added a `Campfire` block state for reading and modifying the four cooking slots of a campfire, with `get_item()` / `set_item()` and `get_cook_time()` / `set_cook_time()`.
...
- Added a `Furnace` block state for reading and modifying furnaces, blast furnaces and smokers. It extends `Container`, so `furnace.inventory` gives the ingredient, fuel and result slots, and `furnace.burn_time` / `furnace.cook_time` match their Bukkit counterparts. Bedrock derives the total cook time from the recipe rather than storing it, so there is no `cook_time_total`.
- Added a `Sign` block state for reading and modifying signs and hanging signs. `block.capture_state()` now returns a `Sign` for these blocks, with `sign.get_side(Sign.Side.FRONT)` giving access to that side's `lines`, `glowing_text` and `color`, and `sign.waxed` controlling whether players can still edit it. Bedrock stores the text color of a sign as a color rather than as one of the dyes, so `color` is a `Color` where Bukkit has a `DyeColor`.
- Added an `ItemFrame` block state for reading and modifying item frames and glow item frames. `block.capture_state()` now returns an `ItemFrame` for these blocks, exposing `item`, `rotation` (a `Rotation`, in 45 degree steps) and `item_drop_chance`. The member names match Bukkit's `ItemFrame`, so a plugin ported from Spigot or Paper reads the same, except that item frames are blocks on Bedrock rather than entities.
...
- **BREAKING**: Actors and players are now handed out as smart handles everywhere, including from events. `ActorEvent.actor` and `PlayerEvent.player` (and every event deriving from them), `PlayerInteractActorEvent.actor`, `PlayerPickupItemEvent.item`, and `BlockBreakEvent.player` / `BlockPlaceEvent.player` return `NotNull`; `ActorKnockbackEvent.source`, `PacketSendEvent.player` and `PacketReceiveEvent.player` return `Nullable`, since those are genuinely absent at times. The collections follow: `Server.online_players`, `Level.actors`, `Dimension.actors`, `BossBar.players` and `PlayerChatEvent.recipients` are now lists of handles, `Dimension.spawn_actor()` returns `Nullable`, and form callbacks receive a handle instead of a raw pointer. C++ plugins use `->` where they used `.`. Holding on to an actor past the event is now safe: the handle keeps the object alive, and touching an actor whose entity is gone raises instead of corrupting memory — check `is_valid` first. Previously the Python bindings handed out non-owning references, so a plugin that stored `event.player` and used it after the actor despawned could crash the server.
- **BREAKING**: The dimension accessors now hand back a smart handle instead of a raw reference or pointer. `Actor::getDimension()`, `Block::getDimension()`, `BlockState::getDimension()` and `Chunk::getDimension()` return `NotNull<Dimension>`; `Level::getDimension()`, `Level::createDimension()` and `MapView::getDimension()` return `Nullable<Dimension>`; `Level::getDimensions()` returns `std::vector<NotNull<Dimension>>`. C++ plugins use `->` where they used `.`, and no longer need `shared_from_this()` to build a `Location` from a dimension. In Python, `Level.get_dimension()`, `Level.create_dimension()` and `MapView.dimension` are now correctly typed as returning `Dimension | None`, so a type checker flags the unchecked `None` that was previously hidden.
- **BREAKING**: `Player::getScoreboard()` returns `NotNull<Scoreboard>` and `Player::setScoreboard()` takes one, and `ServiceManager::registerService()` takes `NotNull<Service>` while `get()`/`load()` return `Nullable<Service>`. In Python, `ServiceManager.load()` is now typed `Service | None`, so a missing provider is visible to a type checker. C++ plugins pass the `shared_ptr` they already hold; it converts on its own.
- **BREAKING**: The command API hands the sender over as a smart handle. `Command::execute()`, `CommandExecutor::onCommand()`, `PluginCommand::execute()`, `CommandMap::dispatch()`, `Server::dispatchCommand()`, and `Command::testPermission()` / `testPermissionSilently()` all take `NotNull<CommandSender>` where they took `CommandSender &`; `CommandSenderWrapper` is built from one and `getWrapped()` returns one; and `Server::getCommandSender()` returns `NotNull<ConsoleCommandSender>`. A C++ plugin must update the first parameter of its `onCommand` override, or it will quietly stop overriding anything, and use `->` where it used `.`. Python plugins are unaffected: `sender` is the same object it always was.
- **BREAKING**: The remaining accessors that handed out a raw reference now use handles too. `Actor::teleport()` takes `NotNull<Actor>`, `BossBar::addPlayer()` / `removePlayer()` and `MapRenderer::render()` take `NotNull<Player>`, `ServiceManager::unregister()` takes `NotNull<Service>`, `Server::createMap()` and `MapView::setDimension()` take `NotNull<Dimension>`, `Objective::getScoreboard()` and `Score::getScoreboard()` return `NotNull<Scoreboard>`, `Dimension::dropItem()` returns `NotNull<Item>`, and `DimensionEvent.dimension`, `PlayerDimensionChangeEvent.from_dimension` / `.to_dimension`, `ScriptMessageEvent.sender` and `ServerCommandEvent.sender` are handles. `Player::getAddress()` returns a reference to the player's address rather than a fresh copy each call. In Python the call sites are unchanged; C++ plugins use `->` where they used `.`.
