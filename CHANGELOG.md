...
- Added `PlayerRecipeBookClickEvent`, called when a player crafts a recipe straight from the recipe book, reporting the `recipe` identifier and whether the player asked for as many copies as the ingredients allow (`make_all`). Cancelling it declines the craft and leaves the ingredients untouched. Ordinary crafting from a grid is reported by `PlayerCraftItemEvent` instead.
