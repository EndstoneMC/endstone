...
- Added a `TileState` block state, mirroring Bukkit's `TileState`. It is the common base of every block state backed by a block entity - `Container`, `Campfire`, `CreatureSpawner`, `ItemFrame` and `Sign` - and `tile_state.is_snapshot` says whether it holds an independent copy of the block entity or points at the one in the world. `Block.capture_state()` takes it: `capture_state()` gives a snapshot, `capture_state(use_snapshot=False)` gives a state that writes straight through to the world.
- Added `Player.send_block_update()`, mirroring Paper's `Player#sendBlockUpdate(Location, TileState)`, which shows a tile state to one player at a location without changing the world for anybody else.
