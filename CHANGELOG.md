...
- `PlayerHideActorEvent` and `PlayerShowActorEvent`, reporting the `actor` whose visibility changed. Neither is called when the actor was already hidden from, or already visible to, that player.
...
- Visibility API: `Player.hide_actor()`, `Player.show_actor()` and `Player.can_see()`. Hiding is tracked per plugin, so an actor stays hidden until every plugin that hid it has shown it again, including across a chunk unload, and disabling a plugin shows again whatever it was hiding. There is no per-actor visible-by-default flag, so `can_see()` reports only whether a plugin is hiding the actor.
- `Player.send_block_change()` for showing a player a block that is not really there. It is sent to that player only and does not change the world.
