...
- nlohmann/json is part of the public C++ API. `endstone_add_plugin()` links it for you, so a plugin can return a `nlohmann::json` from `CustomChart::getChartData()` and read one from a modal form response.
