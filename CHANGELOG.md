...
- `Dimension.get_chunk_at_async()` to request a chunk without blocking and receive it in a callback on a later server tick. Requests belong to a plugin, keep the chunk resident through the callback, and are cancelled when that plugin is disabled. A missing chunk with generation disabled, a load failure or timeout returns `None` to the callback. The default timeout is 1,200 server ticks.
- `Chunk.is_force_loaded` (read/write), `Dimension.is_chunk_force_loaded()`, `Dimension.set_chunk_force_loaded()` and `Dimension.force_loaded_chunks` to manage session-only force loading. Force-loaded chunks stay in memory without nearby players, independently of normal loads and plugin tickets. Loading finishes on a later tick; this does not make chunks tick or preserve the flag across restarts.
...
- `Chunk.entities` (`getEntities()` in C++) to list loaded entities in a chunk, including players; unloaded chunks return an empty list. `Chunk.is_generated` checks whether the chunk is loaded or saved, and `Chunk.is_slime_chunk` checks Bedrock's seed-independent slime chunk pattern. None of these queries loads or generates chunks.
