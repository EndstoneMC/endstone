

# Struct std::formatter&lt; endstone::ItemStack &gt;

**template &lt;&gt;**



[**ClassList**](annotated.md) **>** [**std**](namespacestd.md) **>** [**formatter&lt; endstone::ItemStack &gt;**](structstd_1_1formatter_3_01endstone_1_1ItemStack_01_4.md)








Inherits the following classes: std::formatter< std::string_view >














## Public Types

| Type | Name |
| ---: | :--- |
| typedef [**endstone::ItemStack**](classendstone_1_1ItemStack.md) | [**Type**](#typedef-type)  <br> |




















## Public Functions

| Type | Name |
| ---: | :--- |
|  auto | [**format**](#function-format) (const [**Type**](classendstone_1_1ItemStack.md) & val, FormatContext & ctx) const<br> |




























## Public Types Documentation




### typedef Type 

```C++
using std::formatter< endstone::ItemStack >::Type =  endstone::ItemStack;
```




<hr>
## Public Functions Documentation




### function format 

```C++
template<typename FormatContext>
inline auto std::formatter< endstone::ItemStack >::format (
    const Type & val,
    FormatContext & ctx
) const
```




<hr>

------------------------------
The documentation for this class was generated from the following file `include/endstone/inventory/item_stack.h`

