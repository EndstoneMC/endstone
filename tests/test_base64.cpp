...
TEST(Base64Decode, FailDecodeOneString)
{
    ...
    ASSERT_FALSE(endstone::detail::base64_decode(input).has_value());
}
...
TEST(Base64Decode, DecodesEmptyString)
{
    ...
    std::string expected{};
    auto const actual{endstone::detail::base64_decode("")};
...
TEST(Base64Decode, DecodesZeroArray)
{
    ...
    auto const actual{endstone::detail::base64_decode<std::vector<std::uint8_t>>(input)};
...
TEST(Base64RoundTripTests, AllPossibleBytes)
{
    ...
    auto const encode_string =
        endstone::detail::base64_encode<std::string>(begin(all_possible_bytes), end(all_possible_bytes));
    auto const decoded_bytes = endstone::detail::base64_decode<std::vector<std::uint8_t>>(encode_string);
...
TEST(Base64RoundTripTests, ExhaustiveTests)
{
    ...
    for (auto const &b64_string : base64_strings) {
        auto const decoded = endstone::detail::base64_decode(b64_string);
        ASSERT_TRUE(decoded.has_value());
        auto const encoded_round_trip = endstone::detail::base64_encode(decoded.value());
...
TEST(Base64RoundTripTests, TypeMixTests)
{
    ...
    // str -> str
    {
        auto tmp1 = endstone::detail::base64_encode<std::string>(strinput.begin(), strinput.end());
        ASSERT_EQ(tmp1, stroutput);
        auto tmp2 = endstone::detail::base64_decode<std::string>(stroutput.begin(), stroutput.end());
        ASSERT_EQ(tmp2, strinput);
        auto tmp3 = endstone::detail::base64_encode<std::string>(strinput);
        ASSERT_EQ(tmp3, stroutput);
        auto tmp4 = endstone::detail::base64_decode<std::string>(stroutput);
        ASSERT_EQ(tmp4, strinput);
        auto tmp5 = endstone::detail::base64_encode(strinput);
        ASSERT_EQ(tmp5, stroutput);
        auto tmp6 = endstone::detail::base64_decode(stroutput);
        ASSERT_EQ(tmp6, strinput);
    }

    // str -> u8
    {
        auto tmp1 = endstone::detail::base64_encode<u8vec_type>(strinput.begin(), strinput.end());
        ASSERT_EQ(tmp1, uvecoutput);
        auto tmp2 = endstone::detail::base64_decode<u8vec_type>(stroutput.begin(), stroutput.end());
        ASSERT_EQ(tmp2, uvecinput);
        auto tmp3 = endstone::detail::base64_encode<u8vec_type>(strinput);
        ASSERT_EQ(tmp3, uvecoutput);
        auto tmp4 = endstone::detail::base64_decode<u8vec_type>(stroutput);
        ASSERT_EQ(tmp4, uvecinput);
    }

    // str -> s8
    {
        auto tmp1 = endstone::detail::base64_encode<s8vec_type>(strinput.begin(), strinput.end());
        ASSERT_EQ(tmp1, svecoutput);
        auto tmp2 = endstone::detail::base64_decode<s8vec_type>(stroutput.begin(), stroutput.end());
        ASSERT_EQ(tmp2, svecinput);
        auto tmp3 = endstone::detail::base64_encode<s8vec_type>(strinput);
        ASSERT_EQ(tmp3, svecoutput);
        auto tmp4 = endstone::detail::base64_decode<s8vec_type>(stroutput);
        ASSERT_EQ(tmp4, svecinput);
    }

    // u8 -> str
    {
        auto tmp1 = endstone::detail::base64_encode<std::string>(uvecinput.begin(), uvecinput.end());
        ASSERT_EQ(tmp1, stroutput);
        auto tmp2 = endstone::detail::base64_decode<std::string>(uvecoutput.begin(), uvecoutput.end());
        ASSERT_EQ(tmp2, strinput);
    }

    // u8 -> u8
    {
        auto tmp1 = endstone::detail::base64_encode<u8vec_type>(uvecinput.begin(), uvecinput.end());
        ASSERT_EQ(tmp1, uvecoutput);
        auto tmp2 = endstone::detail::base64_decode<u8vec_type>(uvecoutput.begin(), uvecoutput.end());
        ASSERT_EQ(tmp2, uvecinput);
    }

    // u8 -> s8
    {
        auto tmp1 = endstone::detail::base64_encode<s8vec_type>(uvecinput.begin(), uvecinput.end());
        ASSERT_EQ(tmp1, svecoutput);
        auto tmp2 = endstone::detail::base64_decode<s8vec_type>(uvecoutput.begin(), uvecoutput.end());
        ASSERT_EQ(tmp2, svecinput);
    }

    // s8 -> str
    {
        auto tmp1 = endstone::detail::base64_encode<std::string>(svecinput.begin(), svecinput.end());
        ASSERT_EQ(tmp1, stroutput);
        auto tmp2 = endstone::detail::base64_decode<std::string>(svecoutput.begin(), svecoutput.end());
        ASSERT_EQ(tmp2, strinput);
    }

    // s8 -> u8
    {
        auto tmp1 = endstone::detail::base64_encode<u8vec_type>(svecinput.begin(), svecinput.end());
        ASSERT_EQ(tmp1, uvecoutput);
        auto tmp2 = endstone::detail::base64_decode<u8vec_type>(svecoutput.begin(), svecoutput.end());
        ASSERT_EQ(tmp2, uvecinput);
    }

    // s8 -> s8
    {
        auto tmp1 = endstone::detail::base64_encode<s8vec_type>(svecinput.begin(), svecinput.end());
        ASSERT_EQ(tmp1, svecoutput);
        auto tmp2 = endstone::detail::base64_decode<s8vec_type>(svecoutput.begin(), svecoutput.end());
        ASSERT_EQ(tmp2, svecinput);
    }
}
