...
struct PlayerEnchantOptionsPacketPayload {
    std::vector<ItemEnchantOption> options;  // +0
};
BEDROCK_STATIC_ASSERT_SIZE(PlayerEnchantOptionsPacketPayload, 24, 24);

class PlayerEnchantOptionsPacket : public Packet {
public:
    PlayerEnchantOptionsPacketPayload payload;                            // +48
    SerializationMode serialization_mode{SerializationMode::CerealOnly};  // +72
};
BEDROCK_STATIC_ASSERT_SIZE(PlayerEnchantOptionsPacket, 80, 80);
