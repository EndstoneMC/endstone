...
/** A bStats line chart with one value. */
class SingleLineChart : public CustomChart {
public:
    using ValueCallback = SingleLineChartCallback;

    SingleLineChart(std::string chart_id, ValueCallback get_value)
...
private:
    ...
    void dispatch(detail::ChartVisitor &visitor) override { visitor.visitSingleLineChart(getChartId(), get_value_); }
