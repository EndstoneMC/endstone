 ...
 */
class ActorInsideBlockEvent final : public Cancellable<ActorEvent<Actor>> {
public:
    ENDSTONE_EVENT(ActorInsideBlockEvent);

    ActorInsideBlockEvent(const NotNull<Actor> &actor, std::unique_ptr<Block> block)
        : Cancellable(actor), block_(std::move(block))
    {
    }
    ~ActorInsideBlockEvent() override = default;

    /**
     * Gets the block involved in this event.
     *
     * @return the block containing the Actor
     */
